...
client.on("guildMemberAdd", member => {
	...
	var role = member.guild.roles.find("name", "Kullanıcılar");
	if (!role) return;
...
client.on('guildCreate', guild => {
    let channel = client.channels.get("525035058831294495")
        const embed = new Discord.RichEmbed()
        .setColor("RANDOM")
...
client.on("message", async message => {
 ...
 let prefix = prefixes[message.guild.id].prefixes;
  ...
  if(cooldown.has(message.author.id)){
    message.delete();
    return message.reply("Komutlar arasında 2 saniye beklemelisin.")
  }
